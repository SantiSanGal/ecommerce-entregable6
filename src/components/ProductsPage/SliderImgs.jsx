...
const SliderImgs = ({ product }) => {
    ...
    const handlePrevious = () => {
        if (imgSelected - 1 < 0) {
            ...
            //si se agregaran más imágenes, irá al último
        } else {
    ...
    return (
        <div className='carousel'>
            <div className='slider'>
                <button onClick={handlePrevious} className='slider__btn slider__btn-letf'><i className='bx bx-chevron-left'></i></button>
                <div style={styleMovement} className='slider__movement'>
                    {
                        product?.images.map(image => (
                            <div key={image.url} className='slider__container-img'>
            ...
            <div className='carousel__footer'>
                {
                    product?.images.map((image, index) => (
                        <div 
                            ...
                            //si el indice es igual a la imagen que muestra, agregar la clase active-img
                            onClick={() => setImgSelected(index)}
                            //el índice del map, se pasa al imgSelected para cambiar según lo que se seleccione
                        >
